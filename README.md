...
Initialize a new `ScopeProfiler\ScopeProfiler` instance – either as singleton if you prefer to use
a shared instance or as a new instance if you want to use multiple profilers:
...
You will receive an instance of `ScopeProfiler\Scope` which you can use to measure the duration,
memory usage and memory peak of the action. This can either be done automatically by using the
...
The received object is an instance of `ScopeProfiler\Measurement` which contains the following
properties:
